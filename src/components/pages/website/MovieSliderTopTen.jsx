...
const MovieSliderTopTen = ({title="", isTopTen=true, category}) => {
    ...
    <div className='px-6'>
        ...
        <Slider {...settings}>
            {category?.data.map((item, key) => (
                ...
                <img src={`${devImgPath}/${item.topmovies_image}`} alt="" className='w-[331px] h-[441px] object-cover'/>
                ...
                {isTopTen &&  
                <p className='absolute -bottom-3 -left-5 text-[200px] mb-0 leading-none font-black text-accent counter group-hover:opacity-0'>{item.topmovies_ranking}</p>  
                }   
                </div>
            ))}
...
export default MovieSliderTopTen